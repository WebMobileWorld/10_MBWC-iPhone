...
//        if x.grid.visible && y.grid.visible { drawGrid() }
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            ...
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - dots.outerRadius/2 - CGFloat(error)
    ...
    fileprivate func drawAxes() {
        ...
        path.move(to: CGPoint(x: x.axis.inset + 20, y: height - y.axis.inset))
        path.addLine(to: CGPoint(x: width - x.axis.inset + 0, y: height - y.axis.inset))
        path.stroke()
        
        // draw y-axis
        y.axis.color.setStroke()
        path.move(to: CGPoint(x: x.axis.inset + 20, y: height - y.axis.inset))
        path.addLine(to: CGPoint(x: x.axis.inset + 20, y: y.axis.inset))
        path.stroke()
    }
...
*/
    ...
    fileprivate func drawAreaBeneathLineChart(_ lineIndex: Int) {
        ...
        if data.count > 1 {
            ...
            // move to origin
            path.move(to: CGPoint(x: x.axis.inset + 20, y: self.bounds.height - self.y.scale(0) - y.axis.inset))
            // add line to first data point
            path.addLine(to: CGPoint(x: x.axis.inset + 20, y: self.bounds.height - self.y.scale(data[0]) - y.axis.inset))
            ...
            for index in 1..<data.count {
                ...
                path.addLine(to: CGPoint(x: x1 - CGFloat(error), y: y1))
            }

            // move down to x axis
            path.addLine(to: CGPoint(x: self.x.scale(CGFloat(data.count - 1)) + x.axis.inset + 0, y: self.bounds.height - y.axis.inset))
            // move to origin
            path.addLine(to: CGPoint(x: x.axis.inset  + 20, y: self.bounds.height - y.axis.inset))
            path.fill()
        }
    }
    ...
    fileprivate func drawXGrid() {
        ...
        for i in stride(from: start, through: stop, by: step){
            ...
            path.move(to: CGPoint(x: x1 + 20, y: y1))
            path.addLine(to: CGPoint(x: x1 + 20, y: y2))
        }
        path.stroke()
    }
    ...
    fileprivate func drawYGrid() {
        ...
        for i in stride(from: start, through: stop, by: step){
            ...
            path.move(to: CGPoint(x: x1 + 20, y: y1))
            path.addLine(to: CGPoint(x: x2 + 20, y: y1))
        }
        path.stroke()
    }
    ...
    fileprivate func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerated() {
            ...
            if xAxisData.count == 1 {
                ...
                let label = UILabel(frame: CGRect(x: xValue - CGFloat(error), y: y, width: width, height: x.axis.inset))
                label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
    ...
    fileprivate func drawYLabels() {
        ...
        var yValue: CGFloat
//        let (start, stop, step) = self.y.ticks
        let start = self.getMinimumValue()
        let stop = self.getMaximumValue()
        let step = (stop - start) / 4
        ...
        for i in stride(from: start, through: stop, by: step){
            ...
            if number == 1 {
                let label = UILabel(frame: CGRect(x: 3, y: yValue - 5, width: y.axis.inset + 50, height: y.axis.inset))
                label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
                label.textAlignment = .left
                label.text = String(format: "%.01f", CFloat(i))
                self.addSubview(label)
            }
            else {
                let label = UILabel(frame: CGRect(x: 3, y: yValue, width: y.axis.inset + 50, height: y.axis.inset))
                label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
                label.textAlignment = .left
                label.text = String(format: "%.01f", CFloat(i))
                self.addSubview(label)
